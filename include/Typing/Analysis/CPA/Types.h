...
#ifndef NPC_TYPING_ANALYSIS_CPA_SUPPORT_H
#define NPC_TYPING_ANALYSIS_CPA_SUPPORT_H
...
public:
  ...
  void print(llvm::raw_ostream &os, bool brief = false);
...
private:
  Identifier(llvm::StringRef value) : value(value) {}
...
public:
  ...
  virtual void print(Context &context, llvm::raw_ostream &os,
                     bool brief = false) = 0;
...
public:
  ...
  static bool classof(const ObjectBase *ob) {
    return ob->getKind() >= Kind::FIRST_TYPE &&
           ob->getKind() <= Kind::LAST_TYPE;
  }
  ...
  void print(Context &context, llvm::raw_ostream &os,
             bool brief = false) override;
...
public:
  static bool classof(const ObjectBase *ob) {
    return ob->getKind() == Kind::TypeVar;
  }
  ...
  constructIrType(Context &context, const TypeVarMap &mapping,
                  MLIRContext *mlirContext,
                  std::optional<Location> loc = std::nullopt) override;
...
public:
  static bool classof(const ObjectBase *ob) {
    return ob->getKind() == Kind::CastType;
  }
...
public:
  static bool classof(const ObjectBase *ob) {
    return ob->getKind() == Kind::ReadType;
  }
...
/// Type representing a write-field operation.
/// Referred to as: 'write τ'
class WriteType : public TypeNode {
public:
  static bool classof(const ObjectBase *ob) {
    return ob->getKind() == Kind::WriteType;
  }
...
///   IRValueType: Wrap a primitive MLIR type
...
/// Concreate ValueType that wraps an MLIR Type.
...
public:
  ...
  llvm::ArrayRef<Identifier *> getFieldIdentifier() {
    return llvm::ArrayRef<Identifier *>(fieldIdentfiers, fieldCount);
  }
...
private:
  ObjectValueType(IrTypeConstructor irCtor, Identifier *typeIdentifier,
                  size_t fieldCount, Identifier *const *fieldIdentfiers,
      ...
      : ValueType(Kind::ObjectValueType, 0), irCtor(std::move(irCtor)),
        ...
        fieldIdentfiers(fieldIdentfiers), fieldTypes(fieldTypes) {}

  IrTypeConstructor irCtor;
  Identifier *typeIdentifier;
  size_t fieldCount;
  Identifier *const *fieldIdentfiers;
  TypeNode *const *fieldTypes;
  friend class Context;
};
...
/// Represents an environment scope (i.e. a "countour" in the literature) that
...
public:
  ...
  Constraint *getConstraint(TypeNode *t1, TypeNode *t2) {
    ...
    *it.first = av;         // Replace key pointer with double allocation.
...
private:
  /// Generically creates a uniqueble TypeNode subclass.
...
#endif // NPC_TYPING_ANALYSIS_CPA_SUPPORT_H
